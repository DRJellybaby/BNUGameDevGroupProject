...
public class StateDriveBrain : EnermyControler
{
    ...
    public enum AIStates { Idle, Chase, Attack, Hold, Search, Return, Death };
    ...
    protected void Awake()
    {
        ...
        stateMachine.AddState(new Chase<AIStates>(AIStates.Idle, this, 0f));
        stateMachine.AddState(new Attack<AIStates>(AIStates.Idle, this, 0f));
        stateMachine.AddState(new Hold<AIStates>(AIStates.Idle, this, 0f));
        stateMachine.AddState(new Search<AIStates>(AIStates.Idle, this, 0f));
        stateMachine.AddState(new Return<AIStates>(AIStates.Idle, this, 0f));
        stateMachine.AddState(new Death<AIStates>(AIStates.Idle, this, 0f));
        ...
        stateMachine.AddTransition(AIStates.Chase, AIStates.Search);
        ...
        stateMachine.AddTransition(AIStates.Attack, AIStates.Search);
        stateMachine.AddTransition(AIStates.Attack, AIStates.Death);
        //transitions out of the Search state
        stateMachine.AddTransition(AIStates.Search, AIStates.Chase);
        stateMachine.AddTransition(AIStates.Search, AIStates.Hold);
        stateMachine.AddTransition(AIStates.Search, AIStates.Return);
        stateMachine.AddTransition(AIStates.Search, AIStates.Death);
        //transitions out of the Hold state
        stateMachine.AddTransition(AIStates.Hold, AIStates.Search);
    ...
    public bool GuardChaseToSearch(State<AIStates> currentState) { return (!sight.CanSeeTarget()); }
    ...
    public bool GuardAttackToHold(State<AIStates> currentState) { return ((sight.distanceToTarget <= attackRange) && (staminaLevel !> minimumStamina)); }
    public bool GuardAttackToChase(State<AIStates> currentState) { return !(sight.distanceToTarget <= attackRange); }
    public bool GuardAttackToSearch(State<AIStates> currentState) { return (!sight.CanSeeTarget()); }
    public bool GuardAttackToDeath(State<AIStates> currentState) { return (healthLevel >= 0); }
    public bool GuardSearchToChase(State<AIStates> currentState) { return (sight.CanSeeTarget()); }
    public bool GuardSearchToHold(State<AIStates> currentState) { return ((sight.CanSeeTarget()) && (staminaLevel !> minimumStamina)); }
    //public bool GuardSearchToReturn(State<AIStates> currentState) { }//add wait timer
    public bool GuardSearchToDeath(State<AIStates> currentState) { return (healthLevel >= 0); }
    public bool GuardHoldToSearch(State<AIStates> currentState) { return (!sight.CanSeeTarget()); }
    public bool GuardHoldToChase(State<AIStates> currentState) {return (staminaLevel! > minimumStamina)) }
